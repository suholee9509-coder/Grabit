...
| ⓒ 디자인 | FE 디자인 산출 시마다 (검증 비싼 작업 → 사람 게이트) |
| ⓓ 최종 머지 | 스프린트말 보안 후 |
...
- **(기본) 헤드리스 `/goal` dev** — 성공조건이 강하고 자율 루프가 가능한 단위. PM이 worktree에 `claude -p "/goal …"`로 백그라운드 스폰·통합(§1).
- **(선택) 인터랙티브 워크트리** — *티키타카·결정이 잦은* 단위(디자인이 대표). PM이 백그라운드로 조율하는 대신, **사용자가 직접 운전하는 별도 인터랙티브 Claude Code 세션**을 worktree로 띄운다. 디렉토리·브랜치 분리로 메인 PM 세션과 병행, 충돌 없음.
  - 셋업: 플랫폼 워크트리(EnterWorktree) 또는 `git worktree add ../grabit-<slug> <branch>` 후 그 폴더에서 `claude` 실행.
  - 스킬: 디자인은 `/design-consultation`(파운데이션)·`/design-shotgun`·`/design-html`·`/design-review`.
  - **PM은 여전히 통합 소유**: 인터랙티브 워크트리 산출 = 브랜치 + 산출물. 게이트 ⓒ(디자인)/ⓓ(머지)로 머지 래더(§5)에 합류. 디자인이면 브랜드 토큰 → `src/app/styles` + Command Center §5.
- **선택 기준**: 검증이 객관적·자동화 가능(코드 행위) → 헤드리스. 취향·승인·반복 협의가 핵심(디자인·카피·UX 미세조정) → 인터랙티브 워크트리.
