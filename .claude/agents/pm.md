...
- **순서 = Figma 구조 파악 *먼저* → 그 구조에서 유닛 절단**: ① Figma MCP로 **화면 계층·내비게이션·플로우·컴포넌트 구조**를 깊이 파악(어떻게 연결·이동하는지 — 단순 목록 ❌). ② *그 구조*에서 화면·상태·데이터·컴포넌트를 도출 → 검증 PRD(Command Center §2) + 단위 spec. 기획문서로 의도·스코프·규칙 보강.
- **작업단위 경계 = Figma 구조에서 나온다** (기능 목록 ❌): 단위는 Figma의 화면/플로우 단위로 절단(수직 슬라이스: 화면 + 상태 + 데이터 + 배선), 해당 프레임에 매핑.
- **PM과 frontend 모두 Figma를 본다**: PM은 *구조 파악·유닛 절단·spec 작성*용, frontend는 *픽셀-퍼펙트 구현*용으로 직접 Figma를 연동한다. 둘 다 Figma MCP 필수 (qa는 충실도 검증용).
...
신규 프로젝트면 코드 작성 전에:
1. **Figma 구조 파악 (먼저·필수)**: Figma MCP로 전체를 훑어 **화면 계층·내비게이션·플로우·상태·컴포넌트 구조**를 파악(어떻게 연결·이동하는지까지 — 단순 목록 ❌) → `docs/design/README.md` 프레임 인벤토리 + 링크. **이 구조가 유닛 절단의 1차 기준이다.** 기획 문서(`docs/source/`)로 의도·스코프 흡수·검증.
...
- **단위 = Figma 구조에서 절단** (기능 목록 ❌): 유닛 경계는 *Step 0에서 파악한 Figma 화면/플로우 구조*에서 나온다. 각 단위 = 화면(+상태, 프레임에서 열거) + 데이터 + 배선을 한 슬라이스로, 해당 프레임에 매핑.
