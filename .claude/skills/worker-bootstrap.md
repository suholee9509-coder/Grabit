...
2. gstack healthcheck: `test -d ~/.claude/skills/gstack/bin && echo OK`. **(frontend) Figma MCP 연결 확인** — 미연결 시 status.md escalation.
...
5. goal-loop 진입: 매 턴 spec.md reload(+frontend는 프레임 재확인) · 검증 증거 출력(+frontend는 충실도) · `status.md` 갱신
6. 결정 필요(또는 frontend 디자인 공백) → `status.md`에 escalation 기록 후 정지
